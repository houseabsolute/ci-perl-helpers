...
- If a distro has a script named `dev-bin/install-xt-tools.sh` we now run this before running xt
  tests.
...
- Only pass absolute paths to `prove` when running coverage tests (which are only done on Linux).
